...
% R = 30; % mm
% r = logspace(-3,log10(R),1500)';%logspace(-3,3,1e3)';
% % M = length(r);
% % 
% T = 10; % h
% % t = [0 logspace(-2,log10(T),500)]';%logspace(-3,2,1e2);
% t = logspace(-3,log10(T),200)';%logspace(-3,2,1e2);
% % N = length(t);
...
if index == 1; title('IS- PF-'); end
if index == 2; title('IS- PF+'); end
if index == 3; title('IS+ PF-'); end
if index == 4; title('IS+ PF+'); end
legval = [0.2,0.6,1.8];
...
if index == 1; saveas(f1,[ dir 'figure4b_control'],'pdf'); end
if index == 2; saveas(f1,[ dir 'figure4b_pf'],'pdf'); end
if index == 3; saveas(f1,[ dir 'figure4b_is'],'pdf'); end
if index == 4; saveas(f1,[ dir 'figure4b_ispf'],'pdf'); end
...
f3 = ( (rho3 * exp(g3*B3) .* B3dot)./(rho3 * exp(g3*B3) + 1).^2 ).^(1/6) ...
    .* ( ( rho3 * exp(g3*B3) ) ./( rho3*exp(g3*B3) + 1 ) ).^(1-(1/6));
...
f1 = ( (rho1 * exp(g1*B1) .* B1dot)./(rho1 * exp(g1*B1) + 1).^2 ).^(1/6) ...
    .* ( ( rho1 * exp(g1*B1) ) ./( rho1*exp(g1*B1) + 1 ) ).^(1-(1/6));
...
f2 = ( (rho2 * exp(g2*B2) .* B2dot)./(rho2 * exp(g2*B2) + 1).^2 ).^(1/6) ...
    .* ( ( rho2 * exp(g2*B2) ) ./( rho2*exp(g2*B2) + 1 ) ).^(1-(1/6));
...
f4 = ( (rho4 * exp(g4*B4) .* B4dot)./(rho4 * exp(g4*B4) + 1).^2 ).^(1/6) ...
    .* ( ( rho4 * exp(g4*B4) ) ./( rho4*exp(g4*B4) + 1 ) ).^(1-(1/6));
