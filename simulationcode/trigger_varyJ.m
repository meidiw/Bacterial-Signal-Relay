...
% R = 30; % mm
% r = [0 logspace(-2,log10(R),1e4)]';%logspace(-3,3,1e3)';
% M = length(r);
% 
% T = 10; % h
...
% 1 -> positive feedback (J2), 2 -> trigger wave (J4)
...
if test == 1
    for ii = 1:3
        ...
        solfb = pdepe(m,@(r,t,u,dudx)fb_ode(r,t,u,dudx,D,s0,s1,H1,k,Z2,b2,alpha,lambda2,rho2,g2),...
...
elseif test == 2
    for ii = 1:3
        ...
        solfbqs = pdepe(m,@(r,t,u,dudx)fbqs_pde(r,t,u,dudx,D,s0,H3,k,s3,a3,b4,Z4,alpha,lambda4,rho4,g4),...
            @step_ic,@step_bc,r,t,opts);
        vfbqs = solfbqs(:,:,2)';
...
ylabel('Wavefront-HWHM [mm]','FontSize',20)
if tval == 1; title('IS- PF+'); legendStrings = "$J_2 = $ " + string(J2list); end
if tval == 2; title('IS+ PF+'); legendStrings = "$J_4 = $ " + string(J4list); end
...
if test == 1; saveas(f1,[ dir 'suppfigure_varyJ2'],'pdf'); end
if test == 2; saveas(f1,[ dir 'suppfigure_varyJ4'],'pdf'); end
