...
% 1 -> positive feedback (Z2), 2 -> trigger wave (Z4)
...
if test == 1
    for ii = 1:3
        ...
        solfb = pdepe(m,@(r,t,u,dudx)fb_ode(r,t,u,dudx,D,s0,s1,H1,J2,k,b2,alpha,lambda2,rho2,g2),...
...
elseif test == 2
    for ii = 1:3
        ...
        solfbqs = pdepe(m,@(r,t,u,dudx)fbqs_pde(r,t,u,dudx,D,s0,H3,J4,s3,a3,b4,k,alpha,lambda4,rho4,g4),...
            @step_ic,@step_bc,r,t,opts);
        vfbqs = solfbqs(:,:,2)';
...
if tval == 1; title('IS- PF+'); legendStrings = "$ z_2 = $ " + string(Z2list) + " $\mu$g/mL"; end
if tval == 2; title('IS+ PF+'); legendStrings = "$ z_4 = $ " + string(Z4list) + " $\mu$g/mL"; end
...
if test == 1; saveas(f1,[ dir 'suppfigure_varyz2'],'pdf'); end
if test == 2; saveas(f1,[ dir 'suppfigure_varyz4'],'pdf'); end
